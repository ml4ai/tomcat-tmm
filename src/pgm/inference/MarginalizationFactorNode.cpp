            ...
            if (EXISTS(template_time_step,
                ...
                if (direction == Direction::forward) {
                    if (target_time_step == 0) {
                        ...
                        // At this point, the joint distribution already
                        // contains the full distribution. The nodes in the
                        // joint distribution are not independent anymore after
                        // the time step 0, they will depend on each other as
                    ...
                    for (int row = 0; row < joint_message.get_shape()[1];
                         row++) {
                        Eigen::VectorXd joint =
                            Eigen::VectorXd ::Zero(num_cols);
                        for (int col = 0; col < joint_message.get_shape()[2];
                             col++) {
                            int j =
                                col / ordering_map.right_cumulative_cardinality;
