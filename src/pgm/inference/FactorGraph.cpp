...
namespace tomcat::model {
        ...
        for (const auto& node : dbn.get_nodes_topological_order()) {
            if (!node->get_metadata()->is_parameter()) {
                ...
                if (random_variable->get_metadata()->is_timer()) {
                    ...
                    // There are specific nodes that represent distributions
                ...
                if (random_variable->get_time_step() <= 2) {
                    if (random_variable->has_timer()) {
                        ...
                        if (!random_variable->get_metadata()->is_replicable()) {
                        ...
                        if (random_variable->get_cpd()->is_continuous()) {
                            factor_graph.add_node(
                                node_label,
                                random_variable->get_metadata()
                                    ->get_cardinality(),
                                random_variable->get_time_step(),
                                random_variable->get_cpd()->get_distributions(),
                                ordering_map);
                        }
                        else {
                            factor_graph.add_node(
    ...
    void FactorGraph::create_edges(const DynamicBayesNet& dbn,
        ...
        for (const auto& [source_node, target_node] : dbn.get_edges()) {
            ...
            factor_graph.add_edge(
                source_label, source_time_step, target_label, target_time_step);
                ...
                if (!target_node->get_metadata()->is_replicable()) {
                    ...
                    for (int t = 1; t <= factor_graph.repeatable_time_step -
                        ...
                        string ignore_label = FactorNode::compose_label(
                            compose_intermediary_factor_label(target_label));
    ...
    string FactorGraph::compose_intermediary_label(const string& node_label) {
        return "i:" + node_label;
    }

    string
    FactorGraph::compose_intermediary_factor_label(const string& node_label) {
        stringstream ss;
    ...
    // Member functions
    //----------------------------------------------------------------------
    void FactorGraph::add_non_replicable_node_copy(
        const RVNodePtr& random_variable) {
        const string& node_label = random_variable->get_metadata()->get_label();
        int cardinality = random_variable->get_metadata()->get_cardinality();
        ...
        if (!random_variable->is_segment_dependency()) {
            ...
            if (random_variable->get_parents().empty()) {
                cpd_table = Eigen::MatrixXd::Identity(cardinality, cardinality);
        ...
        for (int t = random_variable->get_time_step() + 1;
             ...
             t++) {
            this->add_node(node_label, cardinality, t, cpd_table, ordering_map);
            ...
            if (!random_variable->is_segment_dependency()) {
                ...
                if (random_variable->get_parents().empty()) {
                    ...
                    new_ordering_map[node_label] = indexing;
                    factor_label =
    ...
    void FactorGraph::add_timed_node(const RVNodePtr& random_variable) {
        const string& timed_node_label =
            random_variable->get_metadata()->get_label();
        int cardinality = random_variable->get_metadata()->get_cardinality();

        int timed_node_id = this->add_variable_node(
            timed_node_label, cardinality, random_variable->get_time_step());
        ...
        CPD::TableOrderingMap total_ordering_map =
            get_segment_total_ordering_map(
    ...
    void FactorGraph::add_node(const string& node_label,
                               int cardinality,
                               int time_step,
                               const vector<DistributionPtr>& distributions,
                               const CPD::TableOrderingMap& cpd_ordering_map) {

        int target_id =
            this->add_target_node(node_label, cardinality, time_step);
        int source_id = this->add_factor_node(
            node_label, time_step, distributions, cpd_ordering_map, node_label);
        boost::add_edge(source_id, target_id, this->graph);
    }

    int FactorGraph::add_target_node(const string& node_label,
                                     int cardinality,
                                     int time_step) {
        ...
        this->repeatable_time_step = max(time_step, this->repeatable_time_step);

        // Include a non factor node in the graph and his parent factor
        // node, that will contain the node's CPD in form of potential
        // function.
        return this->add_variable_node(node_label, cardinality, time_step);
    }
    ...
    int FactorGraph::add_segment_node(const string& node_label, int time_step) {
    ...
    int
    FactorGraph::add_factor_node(const string& node_label,
                                 int time_step,
                                 const Eigen::MatrixXd& cpd_table,
                                 const CPD::TableOrderingMap& cpd_ordering_map,
        ...
        int vertex_id = boost::add_vertex(this->graph);
        this->graph[vertex_id] = make_shared<FactorNode>(node_label,
                                                         time_step,
                                                         cpd_table,
                                                         cpd_ordering_map,
                                                         cpd_owner_label);
    ...
    int FactorGraph::add_segment_marginalization_factor_node(
        ...
        int vertex_id = boost::add_vertex(this->graph);
        this->graph[vertex_id] = make_shared<SegmentMarginalizationFactorNode>(
    ...
    void FactorGraph::add_edge(const string& source_node_label,
        ...
        string target_node_name =
            MessageNode::get_name(target_factor_label, target_node_time_step);
        ...
        if (target_node_time_step > source_node_time_step) {
            shared_ptr<FactorNode> factor_node =
                dynamic_pointer_cast<FactorNode>(this->graph[target_vertex_id]);
        ...
        if (target_node->has_timer()) {
            if (target_node->get_metadata()->get_initial_time_step() ==
                ...
                if (source_node->get_next() == target_node) {
                    ...
                    target_label = SegmentExpansionFactorNode::compose_label(
                        target_node->get_metadata()->get_label());
                }
                else {
    ...
    string FactorGraph::add_intermediary_node(const RVNodePtr& source_node,
        ...
        if (!EXISTS(intermediary_node_name, this->name_to_id)) {
            int cardinality = source_node->get_metadata()->get_cardinality();
            ...
            if (source_node->get_metadata()->has_self_transition()) {
                ...
                boost::add_edge(intermediary_node_id, factor_id, this->graph);
    ...
    void FactorGraph::store_topological_traversal_per_time_step() {
        ...
        using Filtered =
            boost::filtered_graph<Graph, boost::keep_all, function<bool(V)>>;
        ...
        for (int t = 0; t <= this->repeatable_time_step; t++) {
            ...
            boost::topological_sort(time_sliced_graph,
                                    back_inserter(vertex_ids_in_topol_order));

            int num_vertices_in_time_slice = vertex_ids_in_topol_order.size();
    ...
    FactorGraph::get_vertices_topological_order_in(
        ...
        if (from_roots_to_leaves) {
            ...
            return this
                ->time_sliced_reversed_topological_order[relative_time_step];
        }
    }
    ...
    FactorGraph::get_parents_of(const MsgNodePtr& template_node,
        ...
        while (in_begin != in_end) {
            ...
            if (parent_node->get_time_step() < template_node->get_time_step()) {

                transition = true;
                if (time_step > this->repeatable_time_step) {
                    string real_parent_name = MessageNode::get_name(
                        parent_node->get_label(), this->repeatable_time_step);
    ...
    FactorGraph::get_children_of(const MsgNodePtr& template_node,
        ...
        while (out_begin != out_end) {
            ...
            bool transition = false;
            if (template_node->get_time_step() < child_node->get_time_step()) {

                transition = true;
                if (time_step >= this->repeatable_time_step) {
                    string real_child_name = MessageNode::get_name(
                        child_node->get_label(), this->repeatable_time_step);
